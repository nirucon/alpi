...
while [[ $# -gt 0 ]]; do case "$1" in --dry-run) DRY_RUN=1; shift;; -h|--help) usage; exit 0;; *) warn "Unknown arg: $1"; usage; exit 1;; esac; done
run(){ if [[ $DRY_RUN -eq 1 ]]; then say "[dry-run] $*"; else eval "$@"; fi }
backup(){ local f="$1"; [[ -f "$f" ]] || return 0; local b="${f}.bak.$(date +%Y%m%d-%H%M%S)"; sudo cp -a -- "$f" "$b"; warn "Backup: $f -> $b"; }
...
if [[ -f "$ZRAM_CFG" ]]; then
  if grep -q '\[zram0\]' "$ZRAM_CFG" 2>/dev/null; then
    # replace the zram0 block conservatively
    tmp="$(mktemp)"; awk '
      BEGIN{inblk=0}
      /^\[zram0\]/{print "[zram0]"; print "zram-size = ram / 2"; print "compression-algorithm = lz4"; inblk=1; next}
      inblk && /^\[/{inblk=0}
      !inblk{print}
    ' "$ZRAM_CFG" | sudo tee "$tmp" >/dev/null; backup "$ZRAM_CFG"; run "sudo mv '$tmp' '$ZRAM_CFG'"; run "sudo chmod 644 '$ZRAM_CFG'"
  else
    backup "$ZRAM_CFG"; printf '%s' "$ZRAM_DESIRED" | sudo tee -a "$ZRAM_CFG" >/dev/null
  fi
else
  printf '%s' "$ZRAM_DESIRED" | sudo tee "$ZRAM_CFG" >/dev/null; run "sudo chmod 644 '$ZRAM_CFG'"
fi
run "sudo systemctl daemon-reload"; run "sudo systemctl restart systemd-zram-setup@zram0.service || true"

# ───────── journald limits ─────────
step "Tuning systemd-journald"
JOUR_CFG="/etc/systemd/journald.conf.d/99-custom.conf"; run "sudo mkdir -p /etc/systemd/journald.conf.d"
backup "$JOUR_CFG"
...
if ! grep -qE '^tmpfs\s+/tmp\s+tmpfs' "$FSTAB" 2>/dev/null; then
  backup "$FSTAB"; run "echo 'tmpfs /tmp tmpfs defaults,noatime,mode=1777 0 0' | sudo tee -a '$FSTAB' >/dev/null"; warn "Added tmpfs /tmp. Reboot recommended."
else
...
SYSCTL="/etc/sysctl.d/99-swappiness.conf"; backup "$SYSCTL"; run "echo 'vm.swappiness=10' | sudo tee '$SYSCTL' >/dev/null"; run "sudo sysctl -p '$SYSCTL' || true"

# ───────── pacman.conf cosmetics & speed (non-destructive) ─────────
step "Tweaking pacman.conf (Color, ParallelDownloads, ILoveCandy)"
PACMAN_CONF="/etc/pacman.conf"; backup "$PACMAN_CONF"
# Enable Color
if ! grep -qE '^Color' "$PACMAN_CONF"; then run "echo 'Color' | sudo tee -a '$PACMAN_CONF' >/dev/null"; fi
# ParallelDownloads
if ! grep -qE '^ParallelDownloads' "$PACMAN_CONF"; then run "echo 'ParallelDownloads = 5' | sudo tee -a '$PACMAN_CONF' >/dev/null"; fi
# ILoveCandy (fun, optional)
if ! grep -qE '^ILoveCandy' "$PACMAN_CONF"; then run "echo 'ILoveCandy' | sudo tee -a '$PACMAN_CONF' >/dev/null"; fi
